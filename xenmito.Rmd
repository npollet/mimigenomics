...
- one directory containing :
    ...
    - one file with the pseudo_circularised mitochondrial reference genome in bed format as follows:
```
MN259069.1	1	35400	RefSeq	0	+
```    
...
- starting_raw_seq_mtDNA_dir contains fastq read sequence files for reads mapping to mtDNA
    
- starting_raw_seq_long_mtDNA_reads_dir contains 
    - *.long_mtDNA_reads.ids
    - *.long_mtDNA_reads.info 
    - *.small.long_mtDNA_reads.fasta : fasta file of reads covering more than half of the reference mtDNA
    - *.very.long_mtDNA_reads.fasta : fasta file of reads covering more than haf of the reference mtDNA
    
- starting_raw_seq_mapping_ref_dir
    - *.mtDNA.sorted.bam : bam files produced by mapping on the reference mtDNA
    - *.mtDNA.sorted.bam.bai: corresponding bam indices
    - medaka_dir : sub-directory containing medaka outputs
      -barcode*.medaka_variant : sub-directories containing medaka output for each barcode in the form of vcf files
      -final_vcf_dir : all renamed medaka vcf results
